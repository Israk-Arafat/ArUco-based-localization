...
class ARUCOSimulatorNode(Node):
    def __init__(self, *args, **kw):
        ...
        # If no args provided, load from marker map
        if not args:
            import yaml
            import os
            
            # Load marker map
            marker_map_file = self.declare_parameter('marker_map_file', 'marker_map.yaml').value
            show_animation = self.declare_parameter('show_animation', True).value
            ...
            if os.path.exists(config_path):
                with open(config_path, 'r') as f:
                    ...
                    for marker in data.get('markers', []):
                        # Convert 3D position to 2D (x, y, yaw)
                        pos = marker['position']
                        # Extract yaw from quaternion if available
                        quat = marker.get('orientation', [0, 0, 0, 1])
                        # Simple yaw extraction (assumes rotation around z-axis)
                        yaw = 2 * np.arctan2(quat[2], quat[3])
                        marker_list[marker['id']] = (pos[0], pos[1], yaw)
                    
                    # Start robot at a random position
                    x_start = 1.0 + np.random.rand() * 1.0
                    y_start = -1.0 + np.random.rand() * 2.0
                    theta_start = (np.random.rand() * 2 * np.pi) - np.pi
                    start = np.array([x_start, y_start, theta_start])
                    
                    # Goal near one of the markers
                    goal_marker = list(marker_list.values())[0]
                    goal = np.array([goal_marker[0] - 0.5, goal_marker[1], goal_marker[2]])
                    
                    self.get_logger().info(f'Loaded {len(marker_list)} markers from {config_path}')
                    self.get_logger().info(f'Starting at: {start}')
                    
                    self.sim = ARUCOSimulator(
                        start=start,
                        goal=goal,
                ...
                self.get_logger().error(f'Marker map not found: {config_path}')
                # Default setup
                start = np.array([1.0, 0.0, 0.0])
                goal = np.array([3.0, 0.0, 0.0])
                marker_list = {0: (0.0, 0.0, 0.0), 1: (3.0, 0.0, 0.0)}
                self.sim = ARUCOSimulator(
                    start=start,
                    goal=goal,
                    marker_list=marker_list
